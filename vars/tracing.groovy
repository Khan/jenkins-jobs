...
// Anatomy of a trace
//
// Like sysdig, we manage traces by emitting specially formatted lines. In our case, we emit them to
// stdout.
//
// A tracer is akin to a marker for a point in time; and they come in pairs: an entry point and an
// exit point. The span, then, is the unit of time between the entry and exit. Spans can be nested,
// and the root of a span tree is called a trace.
//
...
//
// `<parent id>` is the id of the parent span when you have nested spans. The root span can have an
// empty parent id to indicate it has no children.
//
// `<id>` is the id of the current span. Each span should ideally have a unique id, but the more
// strict requirement is that the pair (parent, child) is unique to avoid mixing two spans
// together.
//
// `<name>` is the name of the span. It's presented in trace viewers and can also be used to look
// at timing across traces for the same type-of-span.
//
// `<args>` is a set of `key=value` pairs, separated by a comma, that can be associated with the span.
// This particular implementation only emits args on the exit line.
class Span implements Serializable {
   ...
   // Returns a 16-byte string of 8 random bytes in hex encoding, suitable for use as a span ID
   @NonCPS
   String makeID() {
...
def withSpan(Map kwargs, Span ctx = null, String name, Closure body) {
   if(ctx == null) {
      ...
      // tree, instead of rooted to a non-existent span
      ctx.ID = "";
   }
