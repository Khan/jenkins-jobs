...
// The tests the workers should run.  Each element is a map:
...
// Run one test-client on a worker machine.
def runTestClient(workerId, clientId) {
   ...
   // others don't have to pay the overhead of starting up jest.
...
def runTestWorker(workerId) {
   ...
   exec(["curl", "--retry", "240", "--retry-delay", "1", "--retry-connrefused",
         "${TEST_SERVER_URL}/begin?worker=client-${workerId}"]);
   try {
      parallel(jobs);
   } finally {
      try {
