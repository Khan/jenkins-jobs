...
<ul>
  ...
  <li> <b>mark-conversions</b>: webapp's services/mark_conversions/. </li>
</ul>
...
// This should be called from within a node().
def deployToMarkConversions() {
   if (!("mark-conversions" in SERVICES)) {
      return;
   }

   withSecrets() {     // TODO(benkraft): do we actually need secrets?
      dir("webapp/services/mark_conversions") {
         sh("./gradlew deploy");
      }
   }
}
...
def deployAndReport() {
    if (SERVICES) {
        parallel(
            ...
            "deploy-to-mark-conversions": { deployToMarkConversions(); },
            "failFast": true,
        );
