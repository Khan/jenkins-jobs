...
def deployToKotlinRoutes() {
   ...
   withSecrets() {     // TODO(benkraft): do we actually need secrets?
      dir("webapp") {
         // HACK: If we sh() in a directory d (or maybe even just dir() to it),
         // jenkins creates a sibling directory d@tmp.  If d is a subdirectory
         // of webapp, this confuses deploy_to_gae.py's local changes check.
         // Instead we have the shell do the cd, which is ugly but works.
