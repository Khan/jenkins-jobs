...
def _computePageWeightDelta() {
   ...
   def script = exec.shellEscapeList(["xvfb-run", "-a", "tools/compute_page_weight_delta.sh 2> /dev/null", GIT_SHA_BASE, GIT_SHA_DIFF]);

   sh(script: "${script} | tee page_weight_delta.txt");
