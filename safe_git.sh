...
_destructive_checkout() {
    ...
    # No need to init (or resync) here, we just want to make sure that
    # when we recurse into changed subrepos, they're at the right version.
    git submodule update --recursive

    # Now we need to update subrepos.  Most subrepos are usually ok,
    # so I use `git status` to only reset the ones that need it.  I
